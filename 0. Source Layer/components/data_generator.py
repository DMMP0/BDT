...
def personalData():  
    ...
    users_information = pd.DataFrame(data=users , columns=['Id Number' , 'first name','last name','sex','DOB','ethnicity','education','purpose','phone number','email'])
    users_information.to_csv('user_information.csv')


def companyData():  
    companyNames = pd.read_excel('datasets/company.xlsx')
    ...
    users_information = pd.DataFrame(data=users , columns=['registeration number' , 'name','establied date','country','number of employes','purpose','phone number','email'])

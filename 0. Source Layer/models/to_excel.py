...
class Excel:
	...
	def bank_excel(self):
		...
		for index,item in bank_data.iterrows():	
			bank_data.loc[index,'open_new_credit_in_6_months'] = random.randint(0,1)
			bank_data.loc[index,'ammount_in_6_months'] = "${:,.2f}".format(val + random.randint(2000,20000))
			bank_data.loc[index,'new_credit_in_12_months'] = random.randint(0,1)
			bank_data.loc[index,'new_credit_in_18_months'] = random.randint(0,1)
			#aggiungere un if statement
			bank_data.loc[index,'ammount_in_12_months'] ="${:,.2f}".format(val + random.randint(2000,20000))
			bank_data.loc[index,'ammount_in_18_months'] = "${:,.2f}".format(val + random.randint(2000,20000))

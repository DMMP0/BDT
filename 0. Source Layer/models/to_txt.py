...
currentDir = os.path.dirname(os.path.abspath(__file__))  # finds the current directory
val = 30000.00
...
class RTF:
    ...
    def create_questura_data(self, questuradata) -> any:
        ...
        questura_data['inscred'] = "${:,.2f}".format(val + random.randint(5_000, 100_000_000_000))
    ...
    def create_risk_broker_data(self, broker_data) -> any:
        ...
        broker_data['insolvent_ammount'] = "${:,.2f}".format(val + random.randint(5_000, 100_000_000_000))
    ...
    def police_rtf(self):
        ...
        for index, item in questura_data.iterrows():
            ...
            if (questura_data.loc[index, 'bankruptcy'] == True):
                questura_data.loc[index, 'inscred'] = "${:,.2f}".format(val + random.randint(500_000, 1_000_000))
    ...
    def bank_rtf(self):
        ...
        for index, item in bank_data.iterrows():
            ...
            bank_data.loc[index, 'ammount_in_6_months'] = "${:,.2f}".format(val + random.randint(5000, 100_000_000_000))
            ...
            bank_data.loc[index, 'ammount_in_12_months'] = "${:,.2f}".format(val + random.randint(2000, 20_000))
            bank_data.loc[index, 'ammount_in_18_months'] = "${:,.2f}".format(
                val + random.randint(5_000, 100_000_000_000))
            bank_data.loc[index, 'house_mortage'] = random.choice([True, False])
            if (bank_data.loc[index, 'house_mortage'] == True):
                bank_data.loc[index, 'amount_of_house_mortage'] = "${:,.2f}".format(
                    val + random.randint(50_000, 500_000))
                bank_data.loc[index, 'amount_duee_mortage'] = "${:,.2f}".format(val + random.randint(50_000, 500_000))
                bank_data.loc[index, 'house_property'] = random.choice([True, False])
            else:
                bank_data.loc[index, 'house_property'] = "${:,.2f}".format(val + random.randint(0, 500_000))
            if (bank_data.loc[index, 'house_property'] is True):
                bank_data.loc[index, 'total_house_amount'] = "${:,.2f}".format(val + random.randint(500, 50_000))
            bank_data.loc[index, 'credit_card_number'] = random.randint(1, 5)
            bank_data.loc[index, 'credit_card_limit_total'] = random.randint(10_000, 500_000)
            bank_data.loc[index, 'actual_debit_credit_cards'] = random.randint(0, 5)
            bank_data.loc[index, 'monthly_income'] = "${:,.2f}".format(val + random.randint(0, 50_000))
            bank_data.loc[index, 'savings'] = "${:,.2f}".format(val + random.randint(1_000, 100_000))
            bank_data.loc[index, 'other_savings'] = "${:,.2f}".format(val + random.randint(100, 20_000))
    ...
    def broker_rtf(self):
        ...
        for index, item in broker_data.iterrows():
            ...
            broker_data.loc[index, 'installment_ammount'] = "${:,.2f}".format(
                val + random.randint(5000, 100_000_000_000))
